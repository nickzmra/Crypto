  ...
  >
    <form @submit.prevent class="flex flex-col w-full p-12 shadow-lg">
      ...
      <input
        ...
        v-model="password"
      />
      <button v-if="form == 'login'" @click="login" class="btn btn-outline bg-transparent mt-8 border-black text-black">
        Login
      </button>
      <button v-else @click="register" class="btn btn-outline mt-8 border-black text-black">
        SignUp
      </button>
